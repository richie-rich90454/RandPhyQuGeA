							...
							<NavLink
								...
								className={({isActive}) =>
									cn(
										...
										isActive ? 'border-primary-600 text-primary-600' : 'border-transparent text-neutral-600 hover:bg-neutral-100 dark:text-neutral-300 dark:hover:bg-neutral-700'
									)
								}
							>
								{item.icon}
			...
			<nav className="fixed inset-x-0 bottom-0 z-10 flex items-center justify-around border-t border-neutral-200 bg-white dark:border-neutral-700 dark:bg-neutral-800">
				{navItems.map(item => (
					<NavLink
						...
						className={({isActive}) =>
							cn('flex flex-1 flex-col items-center gap-1 py-2 text-xs font-medium transition-colors', isActive ? 'text-primary-600' : 'text-neutral-600 dark:text-neutral-300')
						}
					>
						{item.icon}
						<span>{item.label}</span>
