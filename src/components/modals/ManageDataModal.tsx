 ...
 * button clears the entire store. Pagination controls limit the list to
 * PAGE_SIZE records at a time for performance with large histories.
 */
export function ManageDataModal() {
	...
	const handleDeleteAll = () => {
		if (window.confirm('Delete ALL practice data? This cannot be undone.')) {
			clearResults();
			setPage(0);
		}
	};
					...
					<div id="data-list" className="data-list">
						{pageResults.map(result => (
							<div key={result.id} className="data-item">
								...
								<button type="button" className="icon-button data-item-delete" aria-label="Delete record" title="Delete record" onClick={() => deleteResult(result.id)}>
									✕
								</button>
							</div>
						))}
