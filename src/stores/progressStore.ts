...
export const useProgressStore = create<ProgressState>()(
  persist(
    (set, get) => ({
      ...
      getResultsByMode: (mode) =>
        get().results.filter((result) => result.mode === mode),
      ...
      getStats: () => {
        ...
        const totalTimeMs = results.reduce(
          (sum, r) => sum + r.time_taken_ms,
          0,
        );
        const focusedCount = results.filter((r) => r.mode === 'Focused').length;
        ...
        return {
          ...
          focusedCount,
          mentalCount,
          recentResults: (n: number) =>
            [...get().results].reverse().slice(0, n),
          getResultsByMode: (mode: PracticeMode) =>
            get().results.filter((r) => r.mode === mode),
        };
      },
    }),
    {
