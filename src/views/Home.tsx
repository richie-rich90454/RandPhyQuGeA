...
function computeBestStreak(results: PracticeResult[]): number {
	if (results.length === 0) return 0;
	const keys = Array.from(new Set(results.map(r => dateKey(new Date(r.timestamp))))).sort();
	let best = 0;
	let run = 0;
	let prev: Date | null = null;
	for (const key of keys) {
		const cur = new Date(`${key}T00:00:00`);
		const consecutive = prev !== null && Math.round((cur.getTime() - prev.getTime()) / 86_400_000) === 1;
	...
	return (
		<div className="relative h-20 w-20 shrink-0">
			<svg className="h-20 w-20 -rotate-90" viewBox="0 0 64 64" aria-hidden="true">
				<circle cx="32" cy="32" r={radius} fill="none" strokeWidth="6" className="stroke-neutral-200 dark:stroke-neutral-700" />
				<circle
					cx="32"
					cy="32"
					r={radius}
			...
			<div className="absolute inset-0 flex items-center justify-center">
				<span className="text-lg font-semibold text-neutral-900 dark:text-neutral-100">{Math.round(clamped)}%</span>
			</div>
		</div>
	);
}
...
export default function Home() {
		...
		return (
			<div className="mx-auto max-w-5xl space-y-6 p-6 md:p-8">
				...
				<div className="grid grid-cols-1 gap-4 md:grid-cols-3">
					{[0, 1, 2].map(i => (
						<div key={i} className="h-28 animate-pulse rounded-lg bg-neutral-200 dark:bg-neutral-700" />
					))}
				</div>
	...
	const avgDifficulty = hasResults ? results.reduce((sum, r) => sum + r.difficulty, 0) / results.length : 0;

	return (
		<div className="mx-auto max-w-5xl animate-slide-up space-y-6 p-6 md:p-8">
			{/* Greeting */}
			<header>
				<h1 className="text-h1 text-neutral-900 dark:text-neutral-100">{getGreeting()}!</h1>
				<p className="mt-1 text-body text-neutral-500 dark:text-neutral-400">Let&apos;s master some physics today!</p>
			</header>
				...
				<section className="grid animate-fade-in grid-cols-1 gap-4 md:grid-cols-3">
					<Card>
						<CardContent className="flex items-center gap-4">
							...
							<div className="min-w-0">
								<p className="text-small text-neutral-500 dark:text-neutral-400">Current Streak</p>
					...
					<Card>
						<CardContent className="flex items-center gap-4">
							...
							<div className="min-w-0">
								...
								<p className="text-h2 text-neutral-900 dark:text-neutral-100">
									{stats.totalCount} {stats.totalCount === 1 ? 'question' : 'questions'}
								</p>
							</div>
						</CardContent>
					</Card>
					...
					<Card>
						<CardContent className="flex items-center gap-4">
							...
							<div className="min-w-0">
								<p className="text-small text-neutral-500 dark:text-neutral-400">Best Streak</p>
								<p className="text-h2 text-neutral-900 dark:text-neutral-100">
			...
			{hasResults && lastResult && (
				<Card className="animate-fade-in">
					<CardContent className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
						<div className="min-w-0">
							<p className="text-small text-neutral-500 dark:text-neutral-400">Continue Learning</p>
							<p className="mt-1 text-h3 text-neutral-900 dark:text-neutral-100">{lastResult.mode} Practice</p>
							<p className="mt-1 text-body text-neutral-500 dark:text-neutral-400">
								Avg difficulty {avgDifficulty.toFixed(1)} · {stats.totalCount} total
							</p>
						</div>
			...
			{/* Quick actions */}
			<section>
				<h2 className="mb-3 text-h3 text-neutral-900 dark:text-neutral-100">Quick Actions</h2>
								...
								<Card className="h-full transition-all duration-fast ease-standard hover:-translate-y-0.5 hover:shadow-md">
									<CardContent className="flex h-full items-center gap-3">
										...
										<div className="min-w-0 flex-1">
											<p className="text-h4 text-neutral-900 dark:text-neutral-100">{action.title}</p>
											<p className="truncate text-small text-neutral-500 dark:text-neutral-400">{action.subtitle}</p>
										</div>
			...
			{hasResults && (
				<section className="animate-fade-in">
					<h2 className="mb-3 text-h3 text-neutral-900 dark:text-neutral-100">Recent Activity</h2>
					<Card>
								...
								<div key={r.id} className="flex items-center gap-3 py-3 first:pt-0 last:pb-0">
									...
									<div className="min-w-0 flex-1">
										...
										<p className="text-small text-neutral-500 dark:text-neutral-400">
											{formatDuration(r.time_taken_ms)} · {formatRelative(r.timestamp)}
										</p>
									</div>
									<span className={cn('shrink-0 text-small font-medium', r.is_correct ? 'text-success-700 dark:text-success-300' : 'text-error-700 dark:text-error-300')}>
